...
public enum BotState {
    ...
    EnterBirthDate {
        ...
        @Override
        public void enter(BotContext context) {
            sendMessage(context, "Enter your date of birth, please: dd-MM-yyyy");
        }

        @Override
        public void handleInput(BotContext context) {
            String birthDate = context.getInput();

            if (Utils.isBirthDateValid(birthDate)&&Utils.isAgeGood(birthDate)) {
                context.getUser().setBirthDate(Utils.getLocalDateFromString(birthDate));
                next = Approved;
            } else {
                sendMessage(context, "Wrong birthDate!");
                next = EnterBirthDate;
            }
        }
